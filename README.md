...
과거 HD-2D·3D 시험 씬과 전용 자산은 프로젝트에서 삭제했습니다. 실행 가능한 모든 게임 경로는 전체 2D로만 구성합니다.
...
`scenes/`, `scripts/`, `assets/`에는 3D 노드·카메라·조명·메시 또는 3D 모델 파일을 두지 않습니다. `tests/no_3d_content_smoke.gd`가 이 규칙을 회귀 검사합니다.
